        ...
        <div class="row">
            ...
            <div class="span4">
                <ul class="breadcrumb">
                    ...
                    <li class="active">Certidão Quitação ITBI</li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <div class="aligncenter">
            <div class="row justify-content-center">
                <div class="col-md-10">
                    ...
                    <div class="card">
                        <div class="span12 text-left">
                            <table id="imoveis" class="display" style="width:100%; display: none;">
                                <thead>
                                    <tr>
                                        <th>Matrícula</th>
                                        <th>Tipo</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{  utf8_encode(urldecode(iconv('UTF-8', 'ISO-8859-1',$registro->planta))) }} /
                                            {{  utf8_encode(urldecode(iconv('UTF-8', 'ISO-8859-1',$registro->quadra))) }} /
                                            {{  utf8_encode(urldecode(iconv('UTF-8', 'ISO-8859-1',$registro->lote))) }}
                                        </td>
                                        <td>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        ...
        var pesquisaITBITbl = $('#pesquisaITBITbl').DataTable({
            "language": {
            ...
            var table = $('#imoveis').DataTable({
                                ...
                                "targets": -1,
                                "data": null,
                                "defaultContent": "<button>Selecionar</button>"
                            } ]
                        });
            ...
            $('#imoveis tbody').on( 'click', 'button', function () {
                ...
                // var url = "{{URL::to('/certidaoITBIImprimir/:id')}}";
                var url = "{{URL::to('/listaItbiImovel/:id')}}";
                ...
                $.get(url, function (resposta) {
                    ...
                    console.log (obj);
                    if (obj.iStatus == 1) {
                        $('#pesquisaITBITbl').dataTable().fnClearTable();
                        $.each(obj.aListaItbiQuitado, function(i, item) {
                            // alert(item.matricula);
                            var data = item.it01_data.split("-");
                            pesquisaITBITbl.row.add( [
                                data[ 0 ],
                                item.it01_guia,
                                data[2]+ "/" + data[1] + "/" + data[0],
                                '<button type="button" class="btn e_wiggle" onClick="javascript:selecionaPesquisaITBI(' + data[ 0 ]  + ',' + item.it01_guia + ' );"  >Imprimir</button>'
                            ] ).draw( false );
                        });
                        $('#pesquisaITBITbl').css('display', 'none' );
                        $("#pesquisaITBI").modal('toggle');
                        $('#pesquisaITBITbl').css('width', '100%' );
                        $('#pesquisaITBITbl').css('display', 'block' );
                        pesquisaITBITbl.columns.adjust().draw();
                        
                    }
                    else {
                        alert("O Imóvel não possui Nenhuma certidão de Quitação de ITBI.\n\nCódigo: CERT_009");
                        return false;
                    }
                });
            } );
        ...
        function selecionaPesquisaITBI(matricula, guia){
            // alert(matricula);
            // alert(guia);
            var url = "{{URL::to('/certidaoITBIImprimir/:matr/:guia')}}";
            url = url.replace(':matr', matricula);
            url = url.replace(':guia', guia);
